...
tasksRoutes.post("/", (req, res) => {
  ...
  fs.writeFileSync(writePath, JSON.stringify(tasksListUpdated), {
    encoding: "utf8",
    flag: "w",
  });
...
tasksRoutes.delete("/", (req, res) => {
  ...
  let writePath = path.join(__dirname, "..", "tasksList.json");
  fs.writeFileSync(writePath, JSON.stringify(updatedTasksList), {
    encoding: "utf8",
    flag: "w",
  });
...
tasksRoutes.put("/:id", (req, res) => {
  ...
  const readPath = path.join(__dirname, "..", "tasksList.json");
  const tasksList = JSON.parse(fs.readFileSync(readPath, "utf8"));
